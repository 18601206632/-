...
-(instancetype)initWithCompanyModel:(companyModel *)company
{
    if (self=[super init]) {
        _company=company;
    }
...
-(UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[companyDetailCell class] forCellReuseIdentifier:@"Cell"];
    }
    return _tableView;
}
-(UIView *)headerView
...
//    _logoIv.clipsToBounds=YES;
    [_cover setImageWithURL:[NSURL URLWithString:self.company.cover]];
    ...
    _logoIv.clipsToBounds=YES;
    [_logoIv setImageWithURL:[NSURL URLWithString:self.company.logo]];
    ...
    _titleLb.text = self.company.companyName;
    //添加滚动栏
    _areaLb=[UILabel new];
...
//        make.bottom.mas_equalTo(-15);
    ...
    _areaLb.text=self.company.areaDetail;
...
{
    companyDetailCell *cell=[tableView dequeueReusableCellWithIdentifier:@"Cell" forIndexPath:indexPath];
...
-(UIView *)footerView
{
    ...
    self.textView.text=self.company.companyIntroduction;
    return footView;
...
//    UITextView *textView=[[UITextView alloc]initWithFrame:CGRectMake(0, 0, kWindowW, 50) textContainer:contain];
